...
:mod:`vmex.core.transforms`; building them with NumPy avoids eager device
...
def mode_table(mpol: int, ntor: int) -> ModeTable:
    ...
    ``n = -ntor..ntor``.  This matches ``vmex.modes.vmec_mode_table`` and
...
def trig_tables(res: Resolution) -> TrigTables:
    ...
    verbatim from the parity-proven ``vmex.kernels.tomnsp.vmec_trig_tables``.
