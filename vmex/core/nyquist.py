...
``vmex.io.wout_files.{nyquist,jxbforce,mercier,bsubs}`` (validated against
golden VMEC2000 ``wout`` files in ``tests/test_wout_golden.py``),
re-hosted on the core types: :class:`vmex.core.geometry.RealSpaceGeometry`
/ :class:`~vmex.core.geometry.HalfMeshJacobian` supply the parity
geometry channels, :class:`vmex.core.fields.MagneticFields` the half-mesh
...
def bsubs_half_mesh(
    *,
    ...
    rs/zs`` come straight from :func:`~vmex.core.geometry.half_mesh_jacobian`
...
def bsubv_lambda_full_mesh(
    *,
    ...
    :func:`vmex.core.forces.lambda_force_kernels` before its ``-lamscale``
    output scaling.
    """
    s = np.asarray(s, dtype=float)
    ns = int(s.shape[0])
...
def _bsubs_angle_derivatives(
    *,
    ...
    if nnyq > 0 and nnyq <= nmax:
        dmult[:, nnyq] *= 0.5
    if bool(lasym):
        # The lasym output tables carry dnorm = 1/(nzeta*ntheta1); the
        # reduced-grid parity analysis below needs the fixaray.f
        # half-interval dnorm = 1/(nzeta*(ntheta2-1)) = 2/(nzeta*ntheta1),
        # so the reconstructed derivatives come out at physical scale
        # (jxbforce.f parity; see :func:`filter_bsubuv_lasym`).
        dmult = 2.0 * dmult
...
def mercier_and_jxb(
    *,
    ...
    if bool(lasym):
        ...
        # (the physical-scale filter is an idempotent projection, so the
        # second pass changes nothing beyond the Fortran sequence).
