...
runnable script in ``examples/optimization/`` (see :doc:`/tutorials/first-optimization`).
...
stop-gradient by construction.  See :doc:`/explanation/adjoint-gradients` for the formulation and cost
analysis.
...
entry-point family (see *Choosing an entry point* in :doc:`/all-of-vmex`; the
...
instead; see *Validating the gradients: the frozen path* in
...
virtual-casing residual path is FD-validated.)  See
:doc:`/explanation/adjoint-gradients`.
