...
def report(label, equilibrium):
    print(f"[{label}] QS = {float(qs.total(equilibrium)):.4e}, "
          f"f_boot = {float(bootstrap.total(equilibrium)):.4e}, "
          f"beta = {100 * float(equilibrium.wout.betatotal):.3f}%, "
          f"aspect = {float(equilibrium.wout.aspect):.3f}, "
...
for max_mode, max_nfev in zip(MAX_MODES, MAX_NFEV):
    ...
    problem = opt.VmecProblem.from_tuples(inp, objective_function_terms, max_mode=max_mode,
        current_dofs=CURRENT_DOFS, vary_major_radius=VARY_MAJOR_RADIUS, use_ess=True, progress=not ci_smoke)
