...
| 14 | #261 polish memory/AUTO/heartbeat | rebased onto main with #258; the two commits the dead agent never pushed are on it; `benchmarks/polish_memory_w7x.json` (third office run: flat 34.23 GiB killed at the chart, batched 3.05 GiB killed at the chart, shipped policy 3.02 GiB certificate / 15.44 GiB chart, completes) and `benchmarks/polish_cost_office.json` (tokamak 1 126 s, Solov'ev 501 s, QA 87 848 s at driver defaults) are committed; the first run's provenance read dirty because the harness's own `arm_*.json` sat in the checkout (use `--scratch`), and the second run's auto arm died with SIGSEGV in the chart build — an intermittent native crash on the office box, logged | Merge when its lanes are green; then 0.8.2 |
