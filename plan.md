...
| 16 | #280 force-error normalizations (was #275/#279) | rebased onto main after #258, baseline regenerated (two redefined fields), #269's stand-in certificates given the new field, guard widened to the measured 1e-12–4e-12 Mac-vs-x86 spread (rtol 1e-10); GitHub created no workflow run for the branch after its rebase — not for a normal push, an empty commit, a close/reopen, a fresh PR, or a renamed branch (no `github-actions` check suite is ever created for those heads while #261's heads get one), so #275/#279 were closed and #280 carries the same commits | Get one CI run, then merge; it retires every "26-fold" number |
